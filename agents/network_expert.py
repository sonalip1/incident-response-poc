...
def analyze(log_text: str, classification: dict, critique: str = None) -> dict:
    """
    Analyzes network logs as a senior network engineer.
    ...
    """
    system_prompt = (
        "You are a senior network engineer. Focus on network routing, DNS, load balancers, latency, firewalls, "
        ...
        "- \"owner_team\": the team responsible for this issue (e.g. \"Network Ops\", \"SecOps\", \"DevOps\")\n"
        "- \"suggested_fix\": a list of strings representing specific actionable steps to resolve the network issue. "
        "If a past incident from the provided history was helpful and has the same resolution, explicitly prefix or add a note "
        "in that step stating that it leverages a historical resolution (e.g., '[Leveraged from Network History]: ...').\n"
    ...
    # Retrieve relevant network incidents from memory
    similar = get_similar_incidents(log_text, category="network", n=2)
    ...
    try:
        ...
        return {
            ...
            "suggested_fix": [f"Network expert failed to analyze. Error: {str(e)}"],
            "confidence": 0.0,
